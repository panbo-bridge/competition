...
class VBEncoder(nn.Module):
    ...
    def forward(self, sents, feats, visual_attention_mask=None):
        ...
        input_ids = torch.tensor([f.input_ids for f in train_features], dtype=torch.long).cuda()#cuda()
        input_mask = torch.tensor([f.input_mask for f in train_features], dtype=torch.long).cuda()#cuda
        segment_ids = torch.tensor([f.segment_ids for f in train_features], dtype=torch.long).cuda()#cuda
        # assert feats.shape[1] == 36 , "Not Using 36 ROIs, please change the following 2 lines"
        visual_segment_ids = torch.ones(input_ids.shape[0],feats.shape[1],dtype=torch.long).cuda()#cuda()
        v_mask = torch.ones(input_mask.shape[0],feats.shape[1],dtype=torch.long).cuda()#cuda()
