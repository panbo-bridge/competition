...
class Transformer(nn.Module):
    def __init__(self,
        ...
        self.encoder = Encoder(dim_model, num_head, hidden, dropout).to(device)
        self.encoders = nn.ModuleList([
            copy.deepcopy(self.encoder)
            for _ in range(num_encoder)])
        self.fc1 = nn.Linear(seq_len * dim_model, n_class).to(device)

    def forward(self, x):
        # out = self.embedding(x)
        # print(out.shape)#[32, 50, 100, 300]
        # out = F.max_pool2d(out, (out.shape[-2], 1))  #
        # out = out.squeeze()
        # print(out.shape)#[32, 50, 300]
        out = x
